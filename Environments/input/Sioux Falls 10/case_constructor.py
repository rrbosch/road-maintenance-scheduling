"""Construct the **SF-10** small instance case study (campaign E1 family / overhaul item 15).

A Sioux Falls instance one project larger than SF-9 and with a longer horizon, kept small enough
that the branch-and-bound exact solver (`Src/Algorithms/BranchAndBoundSolver.py`) still prunes its
way to the **exact true Pareto front** in well under a minute. The full feasible space is far too
large to enumerate, but B&B's monotone-objective bounds make it tractable, so SF-10 extends the
exact-ground-truth scaling story (SF-8 -> SF-9 -> SF-10) without leaving the enumerable-by-B&B regime.

Design targets (set by the author):
  * **10 projects**, ranked by **Volume^2 / Capacity** (= mean V/C x total volume — the same product
    score used for SF-9 / Anaheim: marginal travel-delay impact when a road is crippled grows with
    both how saturated it is and how much flow it carries).
  * **T = 15 time periods.**
  * **average ~3 projects ongoing simultaneously** -> total work = 3 x 15 = 45 project-periods over
    10 projects -> **mean duration 4.5** periods (Poisson(3.5)+1, clipped [1,8]).
  * **hard due dates in [10, 15]** periods (so T = max(hard due) = 15).
...
Methodology is identical to the SF-9 constructor (see `Sioux Falls 9/case_constructor.py`):
  1. **Projects = the 10 highest-impact undirected roads** (Volume^2/Capacity from a baseline UE
  ...
  2. **`hard due date` is set directly in [10,15]** and `p_decay` is calibrated so late-start risk
     hits 50% exactly at `start = hard_due` (`Tardiness` uses `n = start`); no `find_binomial_n`
     re-derivation (which would drift the deadline out of [10,15]).
...
Run from anywhere:  python "Environments/input/Sioux Falls 10/case_constructor.py"
"""
import csv
import os
import shutil
...
SETTINGS = {
    ...
    'avg time per project (years)': (45 / 10) / 4,  # -> Poisson mean (avg_tpp) = 4.5 periods
    ...
    'min due': 10,                               # hard due dates drawn uniformly in [min_due, max_due]
    'max due': 15,
    'max duration': 8,                           # clip Poisson tail so xu = hard_due - duration + 1 >= 1
    # seed 3 yields mean duration exactly 4.5 (sum 45 -> avg 3.0 ongoing over T=15), deadlines in
    # [10,15] (max 15). T=15, cap 5 -> capacity binds; far too large to enumerate but still solved
    # exactly by the branch-and-bound solver in well under a minute.
    'seed': 3,
}
N_PROJECTS = 10
...
def make_projects(top):
    ...
    n = N_PROJECTS
    avg_tpp = s['avg time per project (years)'] * s['time periods per year']  # 4.5

    # durations ~ Poisson(avg_tpp - 1) + 1  (mean = avg_tpp = 4.5), clipped to [1, max duration]
    ...
    # cost kept for completeness (budget is disabled): scalar Poisson x duration, as in SF-9/SFE
    ...
    general = {
        'budget': s['budget'],
        # T must cover the latest possible finish; pin to max_due so the horizon is the intended 15
...
def report_feasible_space(projects, general, nsamp=400000):
    ...
    The nominal space is far too large to enumerate inline, so the feasible fraction is estimated by
    uniformly sampling start vectors and applying the same constraints the exact/B&B solver uses
